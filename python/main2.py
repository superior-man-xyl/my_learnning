...
def get_mode():
    ...
    if mode == '1':
        ...
        global mes
        mes=message
        print("请输入你的秘钥：")#des的密钥是可变长度的
        key = input().replace(' ','')
        print('您输入的密钥长度为',len(key))
        global g
        g=len(mes)
        ...
        if(way==1):
            ...
            repeat=int(input())
            words = "0123456789"
            r = its.product(words, repeat=repeat)
            dic = open("pass.txt","a")
            for i in r:
                dic.write("".join(i))
                dic.write("".join("\n"))
        ...
        if(way==1):
            ...
            print('请选择字典：1：一号字典 任意按键：二号字典')
            way1=int(input())
            if(way1==1):
                # spam= codecs.open('pass2.txt', encoding='utf-8')
                spam=open('pass2.txt', encoding="utf-8")
                spam=str(spam.read())
                List=spam.split('\n')
                for i in range(len(List)-1):
                    keys=List[i][:16]
                    # keys=str(keys)
                    # t=keys.decode(keys.encode("unicode-escape"))
                    print(keys)
                    # if(keys=='1134567890123456'):
                    #     break
                    print(a)
                    b = decrypt(a,keys)
                    print('解密', b)
                ...
                spam=open('pass1.txt',mode="r", encoding="utf-8")
                spam=str(spam.read())
                List=spam.split('\n')
                for i in range(len(List)-1):
                    keys=List[i][:16]
